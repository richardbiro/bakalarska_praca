...
\textbf{Poznámka 1.5}: Jedno z riešení je $K _{2,3}$, pričom $A^\prime_1 = 4$, $A^\prime_2 = 5$, $B^\prime_1 = 2$, $B^\prime_2 = 6$, $B^\prime_3 = 1$. Toto riešenie vzniklo algoritmickým použitím vety 1.5 na množiny $A = \{2\}$ a $B = \{1,3\}$. \\\\

\textbf{Veta 1.6}: Pre každé  $i,j \in \mathbb{N}, 2 \leq i \leq j, (i, j) \neq (2, 2)$ platí, že graf $K_{i,j}$ je vrcholovo bimagický. \\

\textbf{Dôkaz}: Indukciou vzhľadom na $i,j$. Najprv ukážeme, že grafy $K_{2,j}, K_{3,j}$ a $K_{4,5}$ sú vrcholovo bimagické. \\

Graf $K_{2,n+1}$ pre $n \geq 2$ je vrcholovo bimagický - stačí do prvej partície dať prvky $\frac{n(n+1)}{2} + 1$ a $\frac{n(n+1)(3n^2 - n + 10)}{24}$ a do druhej partície prvky $1$ až $n$ spolu s $\frac{n(n+1)(3n^2 - n + 10)}{24} + 1$. \\
...
\textbf{Veta 1.7}: Jediný kubický graf, ktorý je vrcholovo bimagický, je $K_{3,3}$. \\
...
\textbf{Veta 1.8}: Nech $G$ je vrcholovo bimagický regulárny graf. Potom existuje vrcholovo bimagické ohodnotenie grafu $G$ také, že jeho najmenšia hodnota je 1. \\
...
\textbf{Definícia 1.9}: Nech $G$ je vrcholovo bimagický graf s $n$ vrcholmi. Ak sú vrcholom priradené čísla z množiny $\{1, 2, ... , n\}$, tak $G$ nazveme \textbf{perfektne vrcholovo bimagickým grafom}. \\\\
...
\textbf{Veta 1.10}: Perfektne vrcholovo bimagický kompletný bipartitný graf s $n$ vrcholmi existuje práve vtedy, keď $n = 4k$ alebo $n = 4k-1$ pre $k \in \mathbb{N^+}$. \\
